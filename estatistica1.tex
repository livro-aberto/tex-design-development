...
\noindent
\includegraphics[width=300bp]{{USA.NM.VeryLargeArray.03}.jpg}
...
\hline
\multicolumn{11}{|c|}{Temperatura Máxima Média mensal nos anos 1991-2000 na cidade de Porto Alegre} \\
\hline
\thead
...
\noindent
\includegraphics[width=200bp]{{histograma_2intervalos}.png}
...
\draw (0,0) -- (0,10) node [rotate=90, above, yshift=20, midway, scale=1.4] {frequencia absoluta};
...
\foreach \x/\y in {10/1,17/1,21/1,25/1,26/1,28/2,30/1,33/1,35/2,37/1,39/1,40/1,41/1,42/1,44/1,45/2,47/1,48/2,50/2,52/1,53/2,55/2,57/2,58/1,59/1,60/2,62/2,64/2,65/1,66/1,67/1,68/2,70/1,71/1,72/1,73/1,74/1,75/1,77/1,78/2,80/1,82/2,84/1,86/1,89/2,91/1,94/1,97/1,102/1,109/1} \draw [fill=primario] (\x/5,0) rectangle (\x/5+1/5,\y*5)
;
\end{tikzpicture}
...
\draw (0,0) -- (0,14) node [rotate=90, above, yshift=20, midway, scale=1.4] {frequencia absoluta};
\draw (2,-0.2) -- (22,-0.2) node [below, midway, yshift=-20, scale=1.4] {registros de tempo};
\draw (2,-0) -- (22,-0) ;
...
\foreach \x/\y in {0/3,2.5/5/,5/11,7.5/13,10/14,12.5/10,15/5,17.5/3} \draw [fill=primario] (\x+2,0) rectangle (\x+4.5,\y)
;
\end{tikzpicture}
...
\noindent
\includegraphics[width=400bp]{{exemplo_histograma_areas_1}.png}
...
\hline
\thead
\parbox[c][1cm]{3.45cm}{\centering Intervalo de classe} & \parbox[c]{3.45cm}{\centering frequência absoluta} & \parbox[c]{3.45cm}{\centering frequência relativa} & \parbox[c]{3.45cm}{\centering amplitude do intervalo}\\
\hline
{[} 1 ; 3 {[} & 4 & 0,08 & 2 \\
\hline
{[} 3 ; 5 {[} & 8 & 0,16 & 2 \\
...
\noindent
\includegraphics[width=400bp]{{histograma_incorreto_2}.png}
...
\noindent
\includegraphics[width=400bp]{{histograma_correto}.png}

\caption{Histograma correto}
\label{est1-fig-19}\end{figure}
\end{quote}

Comparando as \hyperref[est1-fig-18]{figuras \ref{est1-fig-18}} e \ref{est1-fig-19}, podemos perceber que a primeira distorce a estrutura da distribuição de frequências, atribuindo pesos maiores aos intervalos de maior amplitude e, menores, aos intervalos de menor amplitude.
...
\includegraphics[width=300bp]{{g_barras_tipo_s_1}.png}
...
\includegraphics[width=300bp]{{histograma_5intervalos}.png}
