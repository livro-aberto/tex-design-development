...
\renewcommand\chapterillustration{./abertura-estatistica2.jpg}\ %Photo by Annie Spratt on Unsplash, https://unsplash.com/photos/MwxZTqG6cUw
...
\sphinxstylestrong{Objetivo Geral do Capítulo}
...
\item {} 
revisão dos conceitos apresentados no capítulo \sphinxstylestrong{A Natureza da Estatística}: parâmetro e estatística, definindo média populacional e média amostral, variância populacional e variância amostral.

\item {} 
apresentação de representação gráfica para variáveis quantitativas conhecida como \sphinxstyleemphasis{boxplot} (gráfico caixa).
...
\sphinxstylestrong{Por que estudar o assunto?}
...
\sphinxstylestrong{Desafios do Capítulo}
\begin{enumerate}
\item {} 
...
\sphinxstylestrong{Conteúdos abordados}
\begin{enumerate}
\item {} 
...
\item {} 
Construção do \sphinxstyleemphasis{boxplot} (gráfico caixa).

\end{enumerate}

\sphinxstylestrong{Pré-requisitos}
...
\sphinxstylestrong{Desdobramentos imediatos}
...
\sphinxstylestrong{Abordagem do Capítulo}
...
\sphinxstylestrong{Estratégia pedagógica}
...
\sphinxstylestrong{Organizando as ideias 1: Medidas de posição} Definições de média; mediana; moda e quartis.

\sphinxstylestrong{Praticando o assunto 1:} Atividades explorando conceitos e propriedades apresentados no organizando as ideias 1.

\sphinxstylestrong{Explorando 2: Medidas de dispersão} Proposição de uma atividade envolvendo dois conjuntos de dados reais, todos com medidas de posição iguais, mas apresentando diferenças em suas distribuições caracterizando a necessidade da medida de dispersão.

\sphinxstylestrong{Organizando as ideias 2: Medidas de dispersão}  Definições de amplitude; distância entre quartis; desvio-médio; variância; desvio padrão e coeficiente de variação.

Nesta seção também  serão retomados os conceitos de parâmetro e estimador, tratados no capítulo \sphinxstylestrong{A Natureza da Estatística} apresentando a definição de variância populacional e amostral, desvio-padrão populacional e amostral e, média populacional e amostral.
...
\sphinxstylestrong{Praticando o assunto 2} Atividades que usam os conceitos e propriedades apresentados no organizando 2 e que buscam dar significado às medidas de dispersão definidas.

\sphinxstylestrong{Explorando 3} Proposição de atividade de construção de representação de dados usando Mínimo, Q1, Mediana, Q3 e Máximo.

\sphinxstylestrong{Organizando as ideias 3}
\begin{enumerate}
\item {} 
Definição do \sphinxstyleemphasis{boxplot} (gráfico caixa) representação gráfica para variáveis quantitativas alternativa ao histograma.
...
\sphinxstylestrong{Praticando o assunto 3}  Proposição de atividades de comparação de grupos, usando o boxplot.

\sphinxstylestrong{Para saber mais}
...
\sphinxstylestrong{Material Suplementar}
...
\sphinxstylestrong{Referências Bibliográficas}
\begin{enumerate}
\item {} 
...
\sphinxstylestrong{Objetivos específicos} Identificar a posição da média e dos quartis no gráfico da distribuição.

\sphinxstylestrong{Observações e sugestões} Nesta atividade serão apresentados conjuntos diferentes de dados envolvendo tempos para completar maratonas. Os dados estão disponíveis neste \sphinxhref{https://ggbm.at/ZhqKD9Nz}{link}. Serão fornecidos os totais para que o cálculo das médias envolva apenas uma divisão e possa ser feito com uma calculadora simples. Pretende-se levar o aluno a perceber que na presença de forte assimetria (histograma alongado à direita ou à esquerda), a média pode não ser a medida mais adequada para representar o conjunto e, com isso, motivar a definição de mediana.

É importante discutir as perguntas na caixa \sphinxstylestrong{Para refletir} em sala de aula com o intuito de que os estudantes percebam a necessidade de tratar previamente dados de determinada natureza antes de usá-los numericamente, como é o caso do tempo considerado em unidades distintas (hora:minuto:segundo).
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{Guia para o cálculo de frequências do histograma}\label{\detokenize{PE104-0:id16}}
\sphinxaftercaption
\begin{tabulary}{\linewidth}[t]{|T|T|}
\hline
...
\noindent\sphinxincludegraphics[width=200bp]{{Histograma_mulheres_resposta_1}.png}
...
\noindent\sphinxincludegraphics[width=200bp]{{Histograma_mulheres_resposta_lineas}.png}
...
\sphinxstylestrong{Objetivos específicos} Usar medidas de posição para a comparação das distribuições de uma mesma variável em dois grupos diferentes.

\sphinxstylestrong{Observações e sugestões} Nesta atividade serão comparados os dados dos 100 melhores tempos na maratona de Nova Iorque/2017 para as categorias homens e mulheres. A tabela com o s 100 melhores tempos em horas para a categoria homens é fornecida. A comparação será feita com base nas medidas de posição média, quartis, mínimo e máximo, que são fáceis de serem determinadas apesar da quantidade de dados ser 100. No caso da média, a soma dos 100 tempos é informada.
\end{sphinxadmonition}
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{Legenda}\label{\detokenize{PE104-2:id5}}
\sphinxaftercaption
\begin{tabulary}{\linewidth}[t]{|T|T|T|}
\hline
&
...
\noindent\sphinxincludegraphics[width=200bp]{{Histograma_homens_resposta}.png}
...
\begin{enumerate}
\item {} 
As escalas horizontal e vertical são coincidentes em ambos os histogramas para permitir a comparação. Isto foi discutido no Capítulo \sphinxstylestrong{A Natureza da Estatística}.

\item {} 
Observando-se os histogramas o maior tempo está na categoria de Triciclo de mão.

\item {} 
A simples visualização pode remeter a estimativa de 4 horas para estas categorias, valor bem maior do que as médias para homens e mulheres (\DUrole{xref,std,std-ref}{ativ-maratona-categoria-homens}). No entanto, este valor excede muito as médias reais, apresentadas. Isto mostra que estimar o centro de equilíbrio de uma distribuição a partir do histograma não é trivial.
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{Cotação das ações das empresas}\label{\detokenize{PE104-3:id3}}
\sphinxaftercaption
\begin{tabulary}{\linewidth}[t]{|T|T|}
\hline
...
\begin{sphinxadmonition}{note}{Observação}{Por que o desvio padrão é preferível ao desvio médio?}
...
\sphinxstylestrong{Objetivos específicos} Comparar diferentes distribuições de uma mesma variável quando separada por grupos.

\sphinxstylestrong{Observações e sugestões}  Nesta atividade serão coletados dados de uma mesma variável que possa ser separada em grupos, com o intuito de comparar as suas medidas de posição e dispersão. Sugerem-se algumas opções, dependendo do tamanho da turma e do contexto escolar, podem até ser escolhidas variáveis distintas para grupos pequenos de alunos, por exemplo, um grupo trabalha com as médias de Matemática, outro grupo trabalha com alturas, etc.
...
\begin{enumerate}
\item {} 
Da figura {\hyperref[\detokenize{PE104-5:fig-resumonartes}]{\sphinxcrossref{\DUrole{std,std-ref}{Estatísticas resumo das Notas de Artes}}}} vemos que \(s\approx 1,96\) e que \(R=8-0,8=7,2\). Pela fórmula apresentada temos \({s}\approx\frac{7,2}{4}=1,8\).
...
\setcounter{enumi}{1}
\item {} 
Da figura {\hyperref[\detokenize{PE104-5:fig-summarymaratonamulheres}]{\sphinxcrossref{\DUrole{std,std-ref}{Estatísticas resumo dos 100 melhores tempos para mulheres - Maratona de Nova Iorque/2017}}}} vemos que \(s\approx 0,1866\) e que \(R=3,085-2,448=0,637\). Pela fórmula apresentada temos \({s}\approx\frac{0,637}{4}=0,15925\).
...
\setcounter{enumi}{2}
\item {} 
Da figura {\hyperref[\detokenize{PE104-5:fig-estrategia}]{\sphinxcrossref{\DUrole{std,std-ref}{Estatísticas resumo das cotações das ação nas Companhias A e B.}}}} vemos que, para a companhia A,  \(s\approx 4,5765\) e que \(R=67-56=11\). Pela fórmula apresentada temos \({s}\approx\frac{11}{4}=2,75\).
...
Da figura {\hyperref[\detokenize{PE104-5:fig-estrategia}]{\sphinxcrossref{\DUrole{std,std-ref}{Estatísticas resumo das cotações das ação nas Companhias A e B.}}}} vemos que, para a companhia B,  \(s\approx 17,3738\) e que \(R=90-33=57\). Pela fórmula apresentada temos \({s}\approx\frac{57}{4}=14,25\).
...
\sphinxstylestrong{Objetivos específicos} Calcular a frequência relativa de dados que caem no intervalo centrado na média mais ou menos dois desvios padrões.

\sphinxstylestrong{Observações e sugestões} Esta atividade será útil no final da próxima seção que trata da construção do boxplot e seus resultados serão retomados adiante. Além disso, será útil na verificação da afirmação feita na atividade anterior de que quando não há valores atípicos, a grande maioria dos dados situa-se entre a média mais ou menos dois desvios padrões.
\end{sphinxadmonition}
...
No caso dos dados da \DUrole{xref,std,std-ref}{ativ-Notas-de-Artes} temos \(\bar{x}=5,93\) e \(s=1,96\) tal que os limites deste intervalo são, respectivamente, 2,01 e 9,85. Portanto, das 35 notas podemos ver que 32 observações caem dentro destes limites, ou equivalentemente,  cerca de 91\% das observações.

No caso dos dados da \DUrole{xref,std,std-ref}{maratona-de-NY} temos \(\bar{x}=2,8697\) e \(s=0,1866\) tal que os limites deste intervalo são, respectivamente, 2,4969 e 3,2429. Portanto, dos 100 tempos podemos ver que 93 caem dentro destes limites, ou equivalentemente,  93\% dos tempos.

No caso dos dados da \DUrole{xref,std,std-ref}{estrategia-de-investimento}, para a companhia A, temos \(\bar{x}=61,5\) e \(s=4,5765\) tal que os limites deste intervalo são, aproximadamente, 52,3 e 70,7. Portanto, das 10 cotações podemos ver que todas caem dentro destes limites, ou equivalentemente,  100\% das cotações.

No caso dos dados da \DUrole{xref,std,std-ref}{estrategia-de-investimento}, para a companhia B, temos \(\bar{x}=61,5\) e \(s=18,3136\) tal que os limites deste intervalo são, aproximadamente,  24,9 e 98,1. Portanto, das 10 cotações podemos ver que todas caem dentro destes limites, ou equivalentemente,  100\% das cotações.
...
\phantomsection\label{\detokenize{PE104-5:ativ-comparacao-dois-tipos-de-bonificacao}}\end{sphinxadmonition}

\begin{sphinxadmonition}{note}{Atividade}{comparação das bonificações nas Notas de Artes}

\begin{sphinxadmonition}{note}{Para o professor}

\sphinxstylestrong{Objetivos específicos} Avaliar o efeito no coeficiente de variação de um conjunto de dados quando realizamos transformações de adição de uma constante e de multiplicação por uma constante.

\sphinxstylestrong{Observações e sugestões} Nesta atividade pretende-se retornar ao item (e) da \DUrole{xref,std,std-ref}{ativ-Notas-de-Artes} quando foi perguntado ao estudante o que ele achava melhor: ganhar um ponto ou um acréscimo de 20\% em sua nota. A ideia será propor a mesma pergunta de um ponto de vista do professor, que prefere que a distribuição das notas apresente o menor coeficiente de variação.
\end{sphinxadmonition}
...
\noindent\sphinxincludegraphics[width=200bp]{{boxplotsHM}.png}
...
\sphinxstylestrong{Objetivos específicos} Comparar diferentes conjuntos de dados, considerando a mesma variável.

\sphinxstylestrong{Observações e sugestões} Nesta atividade retomaremos as quatro categorais da maratona de Nova Iorque para usar o boxplot como esquema gráfico para auxiliar na comparação dos resultados para as diferentes categorias, a saber, homens, mulheres, cadeira de rodas e triciclo de mão. Os dados estão disponíveis neste \sphinxhref{https://ggbm.at/ZhqKD9Nz}{link}.
\end{sphinxadmonition}
...
\begin{enumerate}
\item {} 
Considerando a amplitude amostral é fácil perceber que a maior dispersão ocorre na categoria triciclo de mão. O mesmo vale se considerarmos a distância entre quartis. Pela {\hyperref[\detokenize{PE104-7:fig-medidasresumo4categorias}]{\sphinxcrossref{\DUrole{std,std-ref}{Medidas resumo para as quatro categorias da maratona de Nova Iorque/2017}}}} podemos ver que esta resposta também valerá se considerarmos o desvio padrão.

\item {} 
Pela {\hyperref[\detokenize{PE104-7:fig-boxplotsmaratona}]{\sphinxcrossref{\DUrole{std,std-ref}{Boxplots para os 100 melhores tempos das categorias homens e mulheres e dos melhores tempos das categorias cadeira de rodas e triciclo de mão da maratona de Nova Iorque/2017}}}} podemos ver que a única categoria que não apresentou valores atípicos foi a categoria das mulheres, pois não há pontos destacados no boxplot correspondente às mulheres.
...
Logo, conclui-se que nestas categorias tem-se assimetria à direita acentuada. Observe, que nestes dois casos tem-se que a mediana é menor do que a média. Reveja os histogramas construídos na \DUrole{xref,std,std-ref}{ativ-comparacao-de-diferentes-grupos}.
...
Reveja os histogramas construídos na \DUrole{xref,std,std-ref}{ativ-comparacao-de-diferentes-grupos}.
\end{sphinxadmonition}
...
\sphinxstylestrong{Objetivos específicos} Determinar a média, mediana e variância, a partir de um histograma.

\sphinxstylestrong{Observações e sugestões} Esta atividade pretende mostrar a utilidade das fórmulas apresentadas nesta seção para obter medidas de posição e dispersão, quando não se conhecem os dados separadamente.
\end{sphinxadmonition}
...
Não podemos identificar quem é o valor central ou valores centrais, pois não foi dada a informação do número de candidatos que fizeram a prova. Mas isso não é problema, pois a mediana divide a distribuição em dois intervalos de frequências iguais a 50\%. Logo, precisamos identificar em que intervalo, cairá a mediana e, como apresentado na {\hyperref[\detokenize{PE104-1:sec-organizando1}]{\sphinxcrossref{\DUrole{std,std-ref}{Organizando as ideias: medidas de posição}}}} tomar o ponto médio desta classe como aproximação para o valor da mediana. Observe na figura que a frequência do primeiro intervalo é 0,15; a frequência acumulada, considerando os dois primeiros intervalos é 0,15+0,25=0,40 ainda é menod do que 0,5. Considerando os três primeiros intervalos, a frequência acumulada é 0,4+0,2=0,6. Logo, a mediana está no intervalo delimitado por 4 e 6, de modo que tomamos o ponto médio deste intervalo como uma aproxmação para o valor da mediana, a saber, 5.
...
\noindent\sphinxincludegraphics[width=200bp]{{boxplotexercicio9}.png}
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{soma mínima e exemplo de notas possíveis}\label{\detokenize{PE104-E:id11}}
\sphinxaftercaption
\begin{tabulary}{\linewidth}[t]{|T|T|T|T|T|T|}
\hline
\sphinxmultirow{2}{1}{%
...
Exemplo de notas possíveis
\par
\vskip-\baselineskip\vbox{\hbox{\strut}}\end{varwidth}%
\sphinxstopmulticolumn
\\
\cline{5-6}\sphinxtablestrut{1}&\sphinxtablestrut{2}&\sphinxtablestrut{3}&\sphinxtablestrut{4}&\sphinxstartmulticolumn{2}%
\begin{varwidth}[t]{\sphinxcolwidth{2}{6}}
3o.          \textbar{} 4o.
\par
\vskip-\baselineskip\vbox{\hbox{\strut}}\end{varwidth}%
...
\(8.\) \sphinxstylestrong{a)} Concordo, pois podemos perceber que os comprimentos dos intervalos à direita são maiores: \(\textsf{Q3-mediana}=13-6,5=6,5>\textsf{mediana-Q1}=6,5-6=0,5\), \(\textsf{Max-Q3}=13-9=4>\textsf{Q1-Min}=6-4=2\) e \(\textsf{Max-mediana}=13-6,5=6,5>\textsf{mediana-Min}=6,5=4=2,5\).  \sphinxstylestrong{b)} Concordo, pois há assimetria à direita.  \sphinxstylestrong{c)} Comcordo: este gráfico não nos revela a existência de um intervalo de maior frequência, pois os quatro intervalos nele considerados têm frequências relativas iguais a 0,25. \sphinxstylestrong{d)} Concordo, considerando a aproximação apresentada na \DUrole{xref,std,std-ref}{ativ-aproxima-dpa-usando-R}, dada por \(\frac{R}{4}=\frac{13-4}{4}=\frac{9}{4}=2,25.\) \sphinxstylestrong{e)} Concordo, pois DQ=Q3-Q1=9-6=3 gols. \sphinxstylestrong{f)} Concordo: o gráfico não apresenta pontos destacados. Também podemos verificar que a cerca inferior é dada por \(\textsf{Q}1-1,5\cdot\textsf{DQ}=6-1,5\cdot 3=1,5\) e a cerca superior é dada por \(\textsf{Q}3+1,5\cdot \textsf{DQ}=9+4,5=13,5\). Como o valor mínimo é 4 e, o máximo é 13, conclui-se que não existem valores atípicos. \sphinxstylestrong{g)} Concordo, pois o \sphinxstylestrong{boxplot} agrupou os dados em quatro intervalos de frequências relativas dadas por 0,25, a saber. {[}4 ; 6{[}, {[}6 ; 6,5{[}, {[}6,5 ; 9{[} e {[}9 ; 13{]}. Os valores dentro dos parênteses na expressão indicada correspondem aos pontos médios de cada um destes intervalos. Ou seja, esta média, foi calculada com base na fórmula \(\displaystyle{\sum^4_{i=1}}f_i\cdot\tilde{x}_i\).
...
\item {} 
Nesse caso temos 20 dados, e \(\displaystyle{\sum^{20}_{i=1}}=132+5+6=143\) tal qua a média dos \sphinxstylestrong{recordes} nas 20 Copas do Mundo até 2014 é \(\bar{x}=\frac{143}{20}=7,15\). Para o cálculo do desvio padrão temos que considerar a soma de quadrados dos 20 \sphinxstylestrong{recordes}, a saber, considerando as 20 Copas temos \(\displaystyle{\sum^{20}_{i=1}}x^2_i=1060+5^2+6^2=1060+25+36=1121\). Assim, o desvio padrão amostral é dado por \(\sqrt{\frac{1}{20-1}\left (1121-20\cdot 7,15^2\right )}\approx 2,28\).
...
\noindent\sphinxincludegraphics[width=200bp]{{histograma_turma_manha_1}.png}
...
\noindent\sphinxincludegraphics[width=200bp]{{histograma_turma_tarde_1}.png}
...
\noindent\sphinxincludegraphics[width=300bp]{{notas_turmas_manha_tarde}.png}
...
\begin{enumerate}
\item {} 
Concordo, está de acordo com a regra empírica apresentada na secção \DUrole{xref,std,std-ref}{sec-parasabermais}: o intervalo centrado na média mais ou menos um desvio padrão corresponde às notas entre 400 e 600.

\item {} 
Concordo, está de acorodo com a regra empírica apresentada na secção \DUrole{xref,std,std-ref}{sec-parasabermais}: o intervalo centrado na média mais ou menos dois desvios padrões corresponde às notas entre 300 e 700.
...
\centering
\sphinxcapstartof{table}
\sphinxcaption{Legenda}\label{\detokenize{PE104-E:id15}}
\sphinxaftercaption
\begin{tabulary}{\linewidth}[t]{|T|T|T|T|}
\hline
...
\noindent\sphinxincludegraphics[width=200bp]{{boxplotsmedicamentos}.png}
