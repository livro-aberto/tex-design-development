\chapterillustration{./abertura-computacional}{./abertura-computacional-professor}

\chapterwhat{Pensamento computacional e noções básicas de linguagem de programação.}
...
\chapter{Introdução ao Pensamento Computacional}
\label{computacional-chap}


%%%% Página de Créditos

\autorum{Leonardo Barichello}

\revisorum{Diego Lieban}
\revisordois{Larissa Monzon}
\revisortres{Kátia Rocha}
\versao{1.0}

\autordacapa{Pankaj Patel}{Unsplash}{https://unsplash.com/photos/yEAOfWSdzgM}
...
\begin{figure}[H]
\centering
\includegraphics[width=.2\linewidth]{desconto}
\end{figure}
...
\arrange{Um novo jeito de resolver problemas}
\label{comp-arr1}
...
\includegraphics[width=300bp]{pagina_4}
...
\includegraphics[width=.6\linewidth]{fluxograma_fake}
...
\includegraphics[width=.45\linewidth]{fluxograma1}
\hspace{1em}
\includegraphics[width=.45\linewidth]{fluxograma2}
\end{figure}
...
\arrange{A representação via fluxogramas}
\label{comp-arr2}
...
\begin{figure}[H]
\centering
\includegraphics[width=.8\linewidth]{fluxograma3}
\end{figure}
...
\includegraphics[width=\linewidth, trim = 0cm .7cm 0cm .5cm, clip]{algoritmos1}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{portugol.jpg}
\end{figure}
...
\includegraphics[width=.75\linewidth]{conteineres}
\end{figure}
...
\includegraphics[width=250bp]{van}
\caption{Fonte: \href{desplugada.ime.unicamp.br}{Computação Desplugada}}
\end{figure}
...
\includegraphics[width=250bp]{primos}
\end{figure}
...
Uma solução geral, para  times participantes, é mostrada a seguir. A variável \verb|nfases| permite que a solução seja ajustada para torneios com mais ou menos times participantes, desde que siga a mesma estrutura de chaves.
...
\includegraphics[width=.5\linewidth]{xadrez}
\end{figure}
...
\chapterwhat{Aplicações do pensamento computacional e de linguagens de programação na exploração de problemas matemáticos.}

\chapterbecause{Dominar uma linguagem de programação nos permite explorar e resolver problemas matemáticos de maneiras diferentes. Assim, podemos entender o poder de ferramentas computacionais e porque elas são tão usadas em todas as áreas do conhecimento atualmente.} 
\chapter{Aplicações do Pensamento Computacional}
\label{computacional-chap2}
%fim da capa

\section{Seção 6}

\begin{apresentacao}{Introdução à Parte II}
...
\explore{Usando o computador para jogar moedas}
\label{comp-exp8}
\begin{texto}
{
...
\subsection{Usando o computador}
\label{comp-arr8}
...
\subsection{De volta ao jogo Quantas caras?}
\label{comp-exp9}
...
\end{sugestions}
\begin{answer}{Torres, pontos e coordenadas}
{
\begin{enumerate}
\item Sim.
...
Segundo, não estamos buscando uma solução para o sistema de inequações. Isso significaria encontrar as coordenadas de um ponto que satisfaça a todas elas, ou seja, um ponto que esteja dentro do alcance de todas as torres, o que não é necessário para o nosso problema: o que nos interessa é se o ponto é solução de pelo menos uma das inequações.

\begin{figure}[H]
\centering

\includegraphics[width=.50\linewidth]{p2_sinal_2}
\end{figure}

Por exemplo, na figura acima, o ponto $H$ é uma solução das três inequações que determinam os três círculos mostrados. O ponto $E$ é solução de duas das equações que determinam as circunferências traçadas. Já o ponto $F$ é solução de apenas uma das inequações e o ponto $G$ não é solução de nenhuma das inequações.
...
\begin{task}{Um algoritmo que checa sinal}
...
\includegraphics[width=.5\linewidth]{p2_sinal_3.jpg}
\end{figure}
...
Todos os problemas já propostos nessa olimpíada estão disponíveis em \url{www.olimpiada.ic.unicamp.br/pratique}, organizados por ano, nível e fase. Além disso, o sistema de correção de todos eles pode ser usado para testar as suas próprias soluções.
